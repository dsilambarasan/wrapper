...
function _media_theplatform_mpx_restore_last_notification($account) {
  ...
  if (empty($result_data) || (isset($result_data[0]['type']) && $result_data[0]['type'] == 'Exception')) {
    if (isset($result_data[0]['entry']['description']) && strpos($result_data[0]['entry']['description'], 'no longer available') !== FALSE) {
      watchdog('media_theplatform_mpx', 'Attempt to restore backup last_notification value "@value" for @account failed - sequence ID no longer available (likely over a week old).',
        array('@value' => $backup_last_notification_value, '@account' => _media_theplatform_mpx_account_log_string($account)), WATCHDOG_ERROR);
    }
    else {
      watchdog('media_theplatform_mpx', 'Attempt to restore backup last_notification value "@value" for @account failed - request to validate sequence ID failed.',
        array('@value' => $backup_last_notification_value, '@account' => _media_theplatform_mpx_account_log_string($account)), WATCHDOG_ERROR);
    }
  ...
  _media_theplatform_mpx_set_field($account->id, 'last_notification', $backup_last_notification_value);
  watchdog('media_theplatform_mpx', 'Last notification value restored to "@value" from backup_last_notification_value system variable for @account.',
    array('@value' => $backup_last_notification_value, '@account' => _media_theplatform_mpx_account_log_string($account)), WATCHDOG_WARNING);
...
function media_theplatform_mpx_set_last_notification($account, $last_notification = NULL) {
  ...
  if (empty($last_notification)) {
    watchdog('media_theplatform_mpx', 'Resetting mpx last notification sequence ID for @account.',
      array('@account' => _media_theplatform_mpx_account_log_string($account)), WATCHDOG_INFO);
